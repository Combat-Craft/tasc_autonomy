...
Subscribed topics:
    ...
    - /arm_cam/image/compressed
        Compressed camera image used for panorama capture.
...
Related nodes:
    ...
    - camera node / image republisher
        Provides compressed camera frames on /arm_cam/image/compressed.
...
class PanoramaSweeper(Node):
    ...
    def __init__(self):
        ...
        # Receives compressed camera frames.
        # A frame is only saved when the node is in the CAPTURE state.
    ...
    # -------------------------------------------------
    def heading_to_cardinal(self, angle):
        """
        Converts a numeric heading angle in degrees into an 8-direction cardinal compass label.
        """

        if angle is None:
            return None

        # Normalize angle so negative values or values above 360 still work.
        angle = angle % 360.0
    ...
    def image_callback(self, msg: CompressedImage):
        ...
        A frame is only saved if:
            ...
            3. the compressed image can be decoded successfully
        """
